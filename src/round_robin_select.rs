...
pub struct RoundRobinSelectArray<'a, Fut, const N: usize> {
    poll_first: &'a mut usize,
    inner: [Fut; N],
}
pub fn round_robin_select_array<Fut: Future, const N: usize>(
    poll_first: &mut usize,
    arr: [Fut; N],
) -> RoundRobinSelectArray<'_, Fut, N> {
    assert!(*poll_first < N);
    RoundRobinSelectArray {
        inner: arr,
        poll_first,
    }
}
impl<Fut: Future, const N: usize> Future for RoundRobinSelectArray<'_, Fut, N> {
    type Output = (Fut::Output, usize);
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        // Safety: Since `self` is pinned, `inner` cannot move. Since `inner` cannot
        // move, its elements also cannot move. Therefore it is safe to access
        // `inner` and pin references to the contained futures.
        todo!("Implement the round robin here.");
        let item = unsafe {
